...
# Definindo a configuração de cada máquina
machines = {
  "microk8s1"   => {"memory" => "4096", "cpu" => "2", "ip" => "150", "image" => "centos/7"},
  "microk8s2"   => {"memory" => "4096", "cpu" => "2", "ip" => "151", "image" => "centos/7"},
  "microk8s3"   => {"memory" => "4096", "cpu" => "2", "ip" => "152", "image" => "centos/7"}
}
  ...
  machines.each do |name, conf|
    config.vm.define "#{name}" do |machine|
      ...
      machine.vm.provider "virtualbox" do |vb|
        ...
        vb.cpus = conf["cpu"]
        # Criando Grupo para VMs no VirtualBox
        vb.customize ["modifyvm", :id, "--groups", "/MicroK8S"]
      end
